...
Points = 99;case = 2;N=100; NORM = 3;S= 1000;nk=4;nk1=4 ;h=c(0.2,0.35,0.5);P = 3;Sigma<-diag(P)
...
if(N==400){List.Kern$N.400<-list(result_bbeta_var = result_bbeta_var,result_g = result_g,result.fix =result.fix);N=N+3600;print(N);nk=5;nk1= 5;h=c(0.1,0.2,0.5)}
if(N==200){List.Kern$N.200<-list(result_bbeta_var = result_bbeta_var,result_g = result_g,result.fix =result.fix);N=N+200;print(N);nk=5;nk1= 6;h=c(0.2,0.35,0.5)}
if(N==100){List.Kern$N.100<-list(result_bbeta_var = result_bbeta_var,result_g = result_g,result.fix =result.fix);N=N+100;print(N);nk=5;nk1=6 ;h=c(0.2,0.35,0.5)} 
}
sfStop()
#path <- paste0("/home/csr2/gvcmsim/gvcm_example1_case_2.Rdata")
