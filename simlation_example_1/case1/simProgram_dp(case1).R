...
Points = 99;case = 1;N=100; NORM = 3;S= 1000;nk=5;nk1= 5;h=c(0.1,0.3,0.1);P = 3;Sigma<-diag(P)
...
if(N==400){List.Kern$N.400<-list(result_bbeta_var = result_bbeta_var,result_g = result_g,result.fix =result.fix);N=N+3600;print(N);nk=5;nk1= 4;h=c(0.1,0.25,0.001)}
if(N==200){List.Kern$N.200<-list(result_bbeta_var = result_bbeta_var,result_g = result_g,result.fix =result.fix);N=N+200;print(N);nk=5;nk1= 4;h=c(0.1,0.3,0.1)}
if(N==100){List.Kern$N.100<-list(result_bbeta_var = result_bbeta_var,result_g = result_g,result.fix =result.fix);N=N+100;print(N);nk=5;nk1= 4;h=c(0.1,0.3,0.1)} 
}
sfStop()
#path <- paste0("/home/csr2/gvcmsim/gvcm_example1_case_1.Rdata")
#path <- paste0("/home/liujx/gvcmsim/gvcm_example1_case_1.Rdata")
path <- paste0("/home/LiuJX/gvcmsim/gvcm_example1_case_1.Rdata")
