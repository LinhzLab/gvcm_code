...
vc.hf<-function(h,Data,Beta,dp)
  { #Data = DATA[[sn]];Beta = Para.initial;dp = U.fix
    ...
    for(i in 1:length(dp))
    {
      ...
      W1=cbind(K(h[1],u,dp[i])/(V+10^-5),K(h[2],u,dp[i])/(V+10^-5),K(h[1],u,dp[i])/(V+10^-5),K(h[2],u,dp[i])/(V+10^-5))
      yita=y-g+dg*c(rowSums(x*vc0[,1:P]))
      PJ=t(GMU)%*%(dg^2*GMU*W1) 
      vc1[i,]=solve(PJ+diag(10^-4,2*P))%*%t(GMU*W1)%*%(dg*yita)#} 
    }
    return(vc1)
  }
...
####Fix point to estimate bbeta(U)####
  ...
  bbeta.fix.est<-vc.hf(c(h[1],h[4]),DATA[[sn]],Para.initial,U.fix)       
  for(k in 1:P){
