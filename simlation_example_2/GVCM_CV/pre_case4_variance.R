...
calculate_gvcm_parallel<-function(sn){
  ...
  Para.initial<-list(bbeta=gm1[[sn]][,1:P],g=cbind(gm1[[sn]][,P+1],g.deriv = gm1[[sn]][,P+2]),V = DATA[[sn]]$Variance)
...
####link.fix.est at Points from dp2 in order to plot and Variance.fix.est at Points from dp3
  ...
  if(case==4){
    Variance.fix.est = link.fix.est*(1-link.fix.est);Variance_true_fix <-link_true_fix*(1-link_true_fix) 	
    ...
    Variance.est.P<-approx(DATA[[sn]]$g[,1],Variance.est,xout=dp3,rule=2:1)$y	
