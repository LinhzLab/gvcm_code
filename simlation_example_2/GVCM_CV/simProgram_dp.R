...
library(fda)
library(MASS)
source("pre_case4_variance.R")
source("generate_data_2.R")

Points = 199;N=800; NORM = 3;case= 4; S= 500;nk=6;nk1=5;nk2 = 6;P = 2;Sigma<-diag(P);h=c(0.5,1.8,0.1);
...
if(case==4){
  ...
  if(N==1500){List.Kern$N.1500<-list(result1 = result1,result.fix =result.fix);N=N+500;nk=5;nk1=4;h=c(0.5,1.98,0.05);print(N)}
  if(N==1100){List.Kern$N.1100<-list(result1 = result1,result.fix =result.fix);N=N+400;nk=5;nk1=4;h=c(0.5,1.98,0.05);print(N)}
  if(N==800){List.Kern$N.800<-list(result1 = result1,result.fix =result.fix);h=c(0.5,1.8,0.05);N=N+300;print(N)}
} else {
...
sfStop()
path <- paste0("/home/LiuJX/gvcmsim/ex2_variance_known.Rdata")
#path <- paste0("/home/liujx/gvcmsim/gvcm_example1_case_2.Rdata")
#path <- paste0("/home/LiuJX/gvcmsim/gvcm_example3_300_8000.Rdata")
