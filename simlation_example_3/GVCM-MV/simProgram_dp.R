...
library(fda)
library(MASS)
source("pre_sim_3_case9.R")
source("generate_data_3.R")

Points = 199;N=8000; NORM = 3;S= 500;nk2 = 4;P = 2;Sigma<-diag(P);nk1 = 5;nk =4;h=c(0.2,0.05,0.05)
...
x<-mvrnorm(N, rep(0, P), Sigma);bbeta=matrix(cbind(U^2,cos(0.5*pi*U)),N,P);Xbeta=rowSums(x*bbeta)
...
if(N==15000){List.Kern$N.15000<-list(result1 = result1,result.fix =result.fix,Y_pred);N=N+5000;nk1 = 5;nk =4;h=c(0.2,0.05,0.1);print(N)}
if(N==8000){List.Kern$N.8000<-list(result1 = result1,result.fix =result.fix,Y_pred);N=N+7000;nk1 =5;nk =4;h=c(0.2,0.05,0.1);print(N)}
...
path <- paste0("/home/LiuJX/gvcmsim/gvcm_example3_case9.Rdata")
