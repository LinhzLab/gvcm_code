...
spline.initial<-function(sn){  
  ...
  V <- rep(1,N)
  link_est <- function(alpha_2_0,X_Qta_T){
    B_star <- bSpline(X_Qta_T%*% alpha_2_0,df = nk1,intercept = TRUE)
    alpha_1 <- lm(y~B_star-1)$coefficients
    return(alpha_1)  
  }
  Beta_est <- function(alpha_2_0,alpha_1,X_Qta_T){
    B_star <- bSpline(X_Qta_T%*% alpha_2_0,df = nk1,intercept = TRUE)
    ...
    alpha_2 <- lm((y-intercept)~covariate-1)$coefficients
    return(alpha_2)
  }
  loss_0 <-10^5
  for(k in 1:70){
    alpha_1 <- link_est(alpha_2_0,X_Qta_T)
    alpha_2 <- Beta_est(alpha_2_0,alpha_1,X_Qta_T)
...
calculate_gvcm_parallel<-function(sn){
  ...
  #print(paste0(mean(abs(bbeta.est[,1]-DATA[[1]]$bbeta[,1])),sep=',',mean(abs(bbeta.est[,2]-DATA[[2]]$bbeta[,2]))))
   U.fix.1<-seq(quantile(U,probs = 0.2),quantile(U,probs = 0.8),diff(c(quantile(U,probs = 0.2),quantile(U,probs = 0.8)))/Points)
     for(k in 1:P)
     {
 ...
 # Xbeta.fix<-seq(range(DATA[[sn]]$Xbeta)[1],qrange(DATA[[sn]]$Xbeta)[2],diff(range(DATA[[sn]]$Xbeta))/Points)
  ...
  if('try-error' %in% class(Variance.fix.est)){
  	...
  	#print(paste0(mean(abs( Variance.fix.est-Variance_true_fix))))
