...
library(fda)
library(MASS)
source("pre_sim_3_case10.R")
source("generate_data_3.R")

Points = 199;N=8000; NORM = 3;S= 500;nk2 = 4;P = 2;Sigma<-diag(P); nk1 = 5;nk =4;h=c(0.35,0.75,0.05)#nk1 =10;nk =10;h=c(1,1,0.01)
...
while(N<20001){
  ...
  if(N==15000){List.Kern$N.15000<-list(result1 = result1,result.fix =result.fix,Y_pred);N=N+5000;nk1 = 5;nk =4;h=c(0.25,0.5,0.15);print(N)}
  if(N==8000){List.Kern$N.8000<-list(result1 = result1,result.fix =result.fix,Y_pred);N=N+7000;nk1 =5;nk =4;h=c(0.25,0.5,0.15);print(N)}
