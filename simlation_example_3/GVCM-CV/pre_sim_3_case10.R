...
# first we need to estimate initial value for bbeta
  ...
  V <- DATA[[sn]]$Variance
  #alpha_2_0[1:nk] <- lm(DATA[[sn]]$bbeta[,1]~Bu-1)$coefficients
  #alpha_2_0[-(1:nk)] <- lm(DATA[[sn]]$bbeta[,2]~Bu-1)$coefficients
    loss_0 <-10^5
  for(k in 1:70){
...
##identify for bbeta   
 ...
 # Xbeta.fix<-seq(range(DATA[[sn]]$Xbeta)[1],qrange(DATA[[sn]]$Xbeta)[2],diff(range(DATA[[sn]]$Xbeta))/Points)
  ...
  if('try-error' %in% class(Variance.fix.est)){
  	...
  	Variance.fix.est = Variance.fix.est;
  	...
  	#}
  	Variance_true_fix <- (exp(-link.fix)+1)^2 
